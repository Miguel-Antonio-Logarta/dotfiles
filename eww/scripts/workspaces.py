...
def handle_stacking() -> None:
    pass


def handle_splitting_mode(i3: Connection, e: ModeEvent, eww_data) -> None:
    parent = i3.get_tree().find_focused().parent

    if parent.layout == 'splitv':
        eww_data['splitting_mode'] = SPLIT_VERTICAL
        eww_data['splitting_icon'] = SPLIT_VERTICAL_ICON
    else:
        eww_data['splitting_mode'] = SPLIT_HORIZONTAL
        eww_data['splitting_icon'] = SPLIT_HORIZONTAL_ICON
...
if __name__ == "__main__":
    ...
    eww_data = {
        ...
        "splitting_mode": SPLIT_HORIZONTAL,
        "splitting_icon": SPLIT_HORIZONTAL_ICON
    }
    ...
    i3.on(Event.BINDING, lambda i3, e: handle_splitting_mode(i3, e, eww_data))
